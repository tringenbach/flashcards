...
function getAdditionProblems(mode ) {
  const rv = [];
  for (let i = 0; i <= 10; i++) {
    for (let j = 0; j <= 10; j++) {
      if (mode === 'addition') {
        ...
        const m1 = Math.max(i, j);
        const m2 = Math.min(i, j);
        const solution = m1 - m2;
        rv.push({
          problem: `${m1} - ${m2}`,
          solution
        });
...
function MathProblem({ mode }) {
  const [ streak, setStreak ] = useState(0);
  const problems = getAdditionProblems(mode);
